...
export const useModelState = defineStore('ModelState', () => {
    ...
    const createCurrentModelFasadesData = ({ data, def, fasadeNdx, fasadeCount, productId, umHeight }: { data: number[], def?: boolean, fasadeNdx?: number, fasadeCount?: number | boolean, productId?: number | boolean, umHeight?: number | boolean }) => {
        ...
        data.forEach(facadeId => {
            ...
            if (!haveShowCase && hasGlass && !nestandartIDs.includes(productId) ||
                !haveShowCase && hasGlass && nestandartIDs.includes(productId) && lowUm
            ) return
    ...
    // Список строится и для витрин: там патина хранится только в конфиге (для заказа и цены),
    // без отрисовки. Раньше для продукта с type_showcase функция выходила, не обновив список,
    // и редактор получал патины предыдущего фасада. productId оставлен ради единой сигнатуры createCurrent*
