...
<template>
  <div class="order-address-field">
    <label class="order-address-field__label" :for="`field-${field.CODE}`">
      {{ field.NAME }}
      <sup v-if="field.REQUIED">*</sup>
    </label>

    <input
      class="order-address-field__control"
      :id="`field-${field.CODE}`"
      :name="field.CODE"
      type="text"
...
.order-address-field__label sup {
  color: #da444c;
  font-weight: 700;
}

.order-address-field__control {
  width: 100%;
  height: 42px;
  padding: 0 12px;
...
.order-address-field__control:focus {
  border-color: #4285f4;
