...
export const useOptions = ({ withMechanism = true } = {}) => {
    ...
    // Список механизмов считается по выбранной модели и попутно переписывает её
    // MECHANISM_TEMP. Движку, который только синхронизирует опции, он не нужен
    ...
    // Приводит CONFIG.OPTIONS модели к тому, что показывает панель опций: видимость по
    // фасадам и включение обязательных опций. Сама панель делает это только при сборке
    // списка, поэтому у модели, чью панель не открывали после смены фасада или загрузки
    // проекта, в конфиге, а значит и в корзине, оставался устаревший active
    ...
    const filterGroups = (groups, incomingIds, props, PROPS: TTotalProps = modelState.getCurrentModel?.userData?.PROPS) => {
        ...
        let result = groups.map(group => {
            ...
            // map выше возвращает undefined для опций, которых нет в CONFIG.OPTIONS
    ...
    const resetGlobal = () => {
        ...
        const { PROPS } = modelState.getCurrentModel.userData;
        // Раньше видимость пересчитывала отдельная копия логики filterGroups, которая
        // не включала обязательные опции: ставшая видимой обязательная опция оставалась
        // выключенной в конфиге, пока не откроют панель опций
        syncOptions()
    ...
    const checkAvailable = (options: TRootOptionType, PROPS: TTotalProps = modelState.getCurrentModel.userData.PROPS) => {
        ...
        // больше никогда не могла стать видимой. Видимость считают SHOW_ON_FASADE
        // и REQUIRED_OPTIONS, а checkAvailable отвечает только за CONDITIONS.
