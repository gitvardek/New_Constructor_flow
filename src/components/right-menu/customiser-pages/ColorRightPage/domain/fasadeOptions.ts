...
/**
 * Починка конфига, в котором фрезеровки нет в списке фрезеровок фасада — например, ID витрины,
 * записанный старым удалением витрины. Правила те же, что при сборке модели (FasadeBuilder):
 * фрезеровка — первая из списка; патина — null при запрете фрезой, иначе выбранная, если она
 * допустима для полотна, иначе по умолчанию для фрезеровки. null — починка не нужна
 */
