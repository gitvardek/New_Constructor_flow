...
export class BuildUniversalModule extends BuildProduct {
    ...
    //Приводит сохранённую сетку к текущим материалам боковых стенок.
    ...
    private validateGridWalls(grid: GridModule, CONFIG: THREETypes.TConfig): boolean {
        ...
        // разница достаётся последней секции
        ...
        product_data.sections.forEach((section, secIndex) => {
            ...
            cells?.forEach((cell, cellIndex) => {
                if (cellIndex > 0) {
                    ...
                    // Признак типа лежит на ячейке, под которой стоит полка (см. ShelvesManager.addCell).
                    // Стеклянная уходит как glass_shelf и материал корпуса не получает — цвет ей
                    // задаёт собственная модель товара
                    const isGlassShelf = !!cell.glassShelf

                    // position.y — низ детали (выше к нему прибавляется size.y / 2), поэтому
                    // вычитание толщины держит верх полки на нижней границе ячейки над ней:
                    // у тонкого стекла зазор появляется снизу, а не съедает высоту ячейки
    ...
    buildModulegrid(PROPS: THREETypes.TObject, group: THREE.Object3D, moduleBody: THREE.Object3D, baseOffset: Number = 0) {
        ...
        // С опцией «Без дна» нижняя панель удаляется из геометрии (см. case 5738924 выше),
        // поэтому её толщину прибавлять нельзя: иначе наполнения поднимаются на
        // moduleThickness относительно фасадов, которые считаются от низа модуля.
        // В 2D эта поправка уже есть — FillingsManager:
        // grid.horizont + (grid.noBottom ? 0 : grid.moduleThickness)
