...
export class LegBuilder {
    ...
    getLegLength(props: TTotalProps) {
        ...
        console.log(isLengthLags, isLengthLags?.active, 'isLengthLags')
    ...
    /** Изменяет высоту всех ножек до заданного значения */
    update(params: IncomeOptionData): void {
        const { data, mesh } = params;
        if (!mesh || !data) return

        const height = data.values ? 150 : 100

        const positions = mesh.children.map(leg => leg.position.clone())
        ...
        // Пересоздаём ножки с новой высотой на тех же позициях
        positions.forEach(pos => {
            const leg = this.createLeg(height)
